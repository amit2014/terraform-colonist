...
func TestTerraformCodeRootPaths(t *testing.T) {
	...
	c, err := NewColonyFromConfigFile("fixtures/test-rewrite-code-root-paths/colony.yaml")
...
func TestModulePathCannotEscapeCodeRoot(t *testing.T) {
	...
	c, err := NewColonyFromConfigFile("fixtures/test-module-path-cannot-escape-code-root/colony.yaml")
...
func TestSessionRepoDir(t *testing.T) {
	...
	err = os.Link("fixtures/test-session-repo-dir/colony.yaml", testConfigFilePath)
	require.NoError(t, err)

	// load colony
	c, err := NewColonyFromConfigFile(testConfigFilePath)
	require.NoError(t, err)

	assert.Equal(t, tmpdir, c.config.SessionRepoDir)

	// Check that a session repo dir has been created in the tmpdir
	if !utils.FileExists(filepath.Join(tmpdir, ".tfcolony")) {
		assert.Fail(t, "missing .tfcolony directory")
	}
}

func TestUnmarshalTerraformVersion(t *testing.T) {
	c, err := NewColonyFromConfigFile("fixtures/foosite.yaml")
	require.NoError(t, err)
