...
var (
	// During setup, this is set to the path of the compiled colony
...
// compiles the colony binary and returns the path to it.
func compileColony() (string, error) {
...
func TestMain(m *testing.M) {
	// compile the colony binary so we can execute it during tests
	binary, err := compileColony()
	if err != nil {
...
func runTest(t *testing.T, args []string, fixtureBasePath string, version string) *testResult {
	...
	cmd := exec.Command(colonyBinary, args...)
	cmd.Dir = fixturePath
...
func TestColonyApplyChangesSuccess(t *testing.T) {
	for _, version := range terraformVersionsToTest {
...
func TestColonyPlanSuccessNoChanges(t *testing.T) {
	for _, version := range terraformVersionsToTest {
		t.Run(version, func(t *testing.T) {
...
func TestColonyPlanSuccessChanges(t *testing.T) {
	for _, version := range terraformVersionsToTest {
		t.Run(version, func(t *testing.T) {
...
func TestColonyPlanError(t *testing.T) {
	for _, version := range terraformVersionsToTest {
		t.Run(version, func(t *testing.T) {
...
func TestColonyPlanDetachSuccess(t *testing.T) {
	for _, version := range terraformVersionsToTest {
		t.Run(version, func(t *testing.T) {
			...
			sessionDirs, err := getSessionDirs("/tmp/terraform-tests/plan-detach/.tfcolony")
			require.NoError(t, err)
			...
			_, err = os.Stat(filepath.Join("/tmp/terraform-tests/plan-detach/.tfcolony", sessionDirs[0], "foo/sandbox/terraform.tfstate"))
