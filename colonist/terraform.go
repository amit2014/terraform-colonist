...
import (
	...
	"github.com/uber/terraform-colonist/colonist/terraform"
)
...
func (session *Session) newTerraformSession(execution *boundExecution) (*terraform.Session, error) {
	...
	if terraformVersion != nil {
		terraformPath, err := session.repo.colony.terraformVersions.Get(terraformVersion.String())
		if err != nil {
	...
	if terraform.VersionMatches(terraformVersion, ">= 0.10") {
		if _, exists := os.LookupEnv("TF_PLUGIN_CACHE_DIR"); !exists {
			...
			logger.Trace.Printf("colonist: creating shared plugin directory: %v", pluginDir)
