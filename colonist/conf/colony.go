...
// Colony represents the structure of the YAML configuration for Colonist.
type Colony struct {
	...
	// SessionRepoDir is the path to the directory where Colonist
	// will create the .tfcolony session repo that stores log files and
	...
	// Colony. Defaults to the same directory as the config file.
	TerraformCodeRoot string `json:"terraform_code_root"`

	// Default Terraform configuration for this colony. This
...
// Validate checks the colony configuration is good.
func (conf *Colony) Validate() (errs error) {
