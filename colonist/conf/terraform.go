...
func (conf *Terraform) SetDefaultPath() error {
	// If the existing colony config doesn't specify a Terraform path,
...
func (conf *Terraform) Validate() (errs error) {
	// Version must be set by the time colony runs; however, in the config it
	// can be left blank and colony will detect and autofill the version from
